...
for (modelID in 1:3){ #hydro models
  for(i in 1:10){   #para sets
    for (j in 1:10){  # GCMs
      Qh <- paste(modelnm[modelID],'_P',i,'_M',j,'_00_sr','.txt',sep='')
      data1<-read.table(Qh, header = F)
      Qf1 <- paste(modelnm[modelID],'_P',i,'_M',j,'_45_sr','.txt',sep='')
      data2<-read.table(Qf1, header = F)
      Qf2 <- paste(modelnm[modelID],'_P',i,'_M',j,'_85_sr','.txt',sep='')
      data3<-read.table(Qf2, header = F)
      
      Qh <- paste(modelnm[modelID],'_P',i,'_M',j,'_00_ss','.txt',sep='')
      data4<-read.table(Qh, header = F)
      Qf1 <- paste(modelnm[modelID],'_P',i,'_M',j,'_45_ss','.txt',sep='')
      data5<-read.table(Qf1, header = F)
      Qf2 <- paste(modelnm[modelID],'_P',i,'_M',j,'_85_ss','.txt',sep='')
      data6<-read.table(Qf2, header = F)
      ...
      d3<-data.frame()
      # 
      # d1[1:length(a),1] <- rowMeans(data1)   
      # d2[1:length(a),1] <- rowMeans(data2) 
      # d3[1:length(a),1] <- rowMeans(data3) 
      # 
      # z1 <- aggregate(zoo(d1[,1]), rep(c(1:12),20), mean) 
      # z2 <- aggregate(zoo(d2[,1]), rep(c(1:12),20), mean) 
      # z3 <- aggregate(zoo(d3[,1]), rep(c(1:12),20), mean) 
      
      d1[1:length(a),1] <- rowMeans(data1)[13:240]+rowMeans(data4)[13:240]
      d2[1:length(a),1] <- rowMeans(data2)[13:240]+rowMeans(data5)[13:240]
      d3[1:length(a),1] <- rowMeans(data3)[13:240]+rowMeans(data6)[13:240]
      ...
      write.table(deltaq1, 'tr_monthly_uncertain_19.txt', sep = "\t",
