...
data_txt<-'ws_length_change.txt'
data = read.table(data_txt, header = FALSE)
uncertainty_txt = 'ws_length_uncertainty.txt'
uncertaity = read.table(uncertainty_txt, header = FALSE)*100.0   # to percent

#tiff("D:/UncertaintyPaper/paper4_revision/graphs/figure7.tiff", 
     #height = 17.7, width = 22.9, units = 'cm', res = 1200)
windows(9,7)
par(mfrow=c(2,1))
par(mar=c(1,2,0.5,1.0),mgp=c(2.2,0.8,0),family='serif')

boxplot(data[,showWID],outline=FALSE,xaxt='n',yaxt='n', ann=FALSE,tck=-0.01,ylim=c(-80,80))
abline(h=0,lty=2,col='grey40')
axis(2,at=c(-80,-60,-40,-20,0,20,40,60,80),pos=-0.6,labels=c(-80,-60,-40,-20,0,20,40,60,80),tck=-0.01)
