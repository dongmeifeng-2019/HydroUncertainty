...
error_RCM$mean = error_RCM[,14]
error_STP$mean = error_STP[,14]
error_VIC$mean = error_VIC[,14]
...
plot(dates[b],Q_STP$X11119745[b],type='l',col='darkgreen',ylim=c(0,20),
     frame.plot = FALSE,cex.lab=1.2,axes = FALSE,family='serif' ,xlab= '',lty=4,lwd=1.5,
     ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),xaxt="n")
lines(dates[b],Q_RCM$X11119745[b],col='red',lty=2,lwd=1.5)
lines(dates[b],Q_VIC$X11119745[b],col='blue',lty=3,lwd=1.5)
points(dates[b],Q_gauge[b,2],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
...
plot(x1[y1],Peak1[y1,2], ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),
...
lines(x1[y1],Peak3[y1,2],col='red',lty=2,lwd=1.5)
lines(x1[y1],Peak2[y1,2],col='blue',lty=3,lwd=1.5)
points(x1[y1],Peak4[y1,2],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
...
#arrows(2003.5,17.8,2004.7,16,col='red',length = 0.05, angle = 20,lwd =2)
#text(1997.3,36.2,'1/10/1995',col='red')
#text(2003,19.2,'1/9/2005',col='red')
text(x=1986,y=38,labels='Annual Peak',font=2,cex=1.2)
text(x=1989.4,y=28,labels='Calibration (WY 1985-1986,1999-2005)', cex=1.2)
...
text(x=2009.5,y=28,labels='Validation (WY 2006-2013)',cex=1.2)
text(x=2008.6,y=22,labels='RCM-HRR: NSE=0.58', cex=1.0)
text(x=2008.56,y=25,labels='STP-HRR: NSE=0.62',     cex=1.0)
text(x=2008.45,y=19,labels='VIC-HRR: NSE=0.55',    cex=1.0)
text(x=2013,y=39,labels='(c)',font=2,cex=1.2)
par(mar=c(2.0,3.6,0.0,1.1),mgp=c(2,0.8,0),family = "serif")
plot(x1[y1],mean1[y1,2], ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),
     col='darkgreen',lty=4,cex.lab=1.2, cex.axis=1.0,font.lab=1,lwd=1.5,type='l',ylim=c(0,1),
     frame.plot = FALSE,xaxt="n",xlim=c(1985,2013),axes = FALSE)
lines(x1[y1],mean3[y1,2],col='red',lty=2,lwd=1.5)
lines(x1[y1],mean2[y1,2],col='blue',lty=3,lwd=1.5)
points(x1[y1],mean4[y1,2],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
axis(side=1, pos = 0.0,cex.axis=1.0,tck=-0.01,at=c(1985,1989,1993,1997,2001,2005,2009,2013), 
     labels=c("1985","1989","1993",'1997','2001','2005','2009','2013'),family = 'serif')
axis(side=2, cex.axis=1.0,tck=-0.01,family = 'serif')
