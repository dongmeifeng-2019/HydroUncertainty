...
error_RCM$mean = error_RCM[,26]
error_STP$mean = error_STP[,26]
error_VIC$mean = error_VIC[,26]
...
x1 = c(1984:2013)
y1 = c(2:30)
tiff("graphs/figureS3.tiff",  height = 17.78, width = 17.78, units = 'cm', res = 1200)
#windows(7,7)
par(mfrow=c(2,2))
...
plot(dates[b],Q_STP$X11120000[b],type='l',col='darkgreen',ylim=c(0,60),
     frame.plot = FALSE,cex.lab=1.2,axes = FALSE,family='serif' ,xlab= '',lty=4,lwd=1.5,
     ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),xaxt="n")
lines(dates[b],Q_RCM$X11120000[b],col='red',lty=2,lwd=1.5)
lines(dates[b],Q_VIC$X11120000[b],col='blue',lty=3,lwd=1.5)
points(dates[b],Q_gauge[b,5],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
...
axis(side=2, cex.axis=1.0,tck=-0.01,family = 'serif')
abline(h=0.0)
text(dates[b[length(b)-3]],y=60,labels='(b)',font=2,cex=1.2)
...
plot(x1[y1],Peak1[y1,5], ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),
     col='darkgreen',lty=4,cex.lab=1.2, cex.axis=1.0,font.lab=1,lwd=1.5,type='l',ylim=c(0,100),
     frame.plot = FALSE,xaxt="n",xlim=c(1985,2013),axes = FALSE)
lines(x1[y1],Peak3[y1,5],col='red',lty=2,lwd=1.5)
lines(x1[y1],Peak2[y1,5],col='blue',lty=3,lwd=1.5)
points(x1[y1],Peak4[y1,5],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
...
axis(side=2, cex.axis=1.0,tck=-0.01,family = 'serif')
abline(h=0.0)
segments(2005.5, 0.01, x1 = 2005.5, y1 = 99,col = 'grey30', lty = 2)
arrows(1990.8,40.8,1991.8,33.5,col='blue',length = 0.05, angle = 20,lwd =2)
arrows(1994.5,49,1993.2,39,col='blue',length = 0.05, angle = 20,lwd =2)
arrows(1996,72,1995.2,62,col='red',length = 0.05, angle = 20,lwd =2)
arrows(1999.8,64.8,2000.8,57.5,col='blue',length = 0.05, angle = 20,lwd =2)
arrows(2003.5,87.8,2004.7,84.3,col='red',length = 0.05, angle = 20,lwd =2)
text(1990.8,45.2,'2/12/1992',col='blue')
text(1995.8,49,'3/25/1993',col='blue')
text(1996.8,76,'1/10/1995',col='red')
text(1999.8,68.2,'3/5/2001',col='blue')
text(2003,91.5,'1/9/2005',col='red')
text(x=1986,y=98,labels='Annual Peak',font=2,cex=1.2)
text(x=1987.8,y=88,labels='Calibration (WY 1985-2005)', cex=1.2)
text(x=1986.85,y=81.5,labels=paste('RCM-HRR: NSE=',max(error_RCM$best),sep=''), cex=1.0)
text(x=1986.8,y=75,labels=paste('STP-HRR: NSE=',max(error_STP$best),sep=''),     cex=1.0)
text(x=1986.7,y=68.5,labels=paste('VIC-HRR: NSE=',max(error_VIC$best),sep=''),    cex=1.0)

text(x=2009.5,y=88,labels='Validation (WY 2006-2013)',cex=1.2)
text(x=2008.6,y=81.5,labels='RCM-HRR: NSE=0.42', cex=1.0)
text(x=2008.56,y=75,labels='STP-HRR: NSE=0.56',     cex=1.0)
text(x=2008.45,y=68.5,labels='VIC-HRR: NSE=0.47',    cex=1.0)
text(x=2013,y=99,labels='(c)',font=2,cex=1.2)
par(mar=c(2.0,3.6,0.0,1.1),mgp=c(2,0.8,0),family = "serif")
plot(x1[y1],mean1[y1,5], ylab=expression(paste('Discharge (m'^3,'/s)',sep='')),
     col='darkgreen',lty=4,cex.lab=1.2, cex.axis=1.0,font.lab=1,lwd=1.5,type='l',ylim=c(0,2),
     frame.plot = FALSE,xaxt="n",xlim=c(1985,2013),axes = FALSE)
lines(x1[y1],mean3[y1,5],col='red',lty=2,lwd=1.5)
lines(x1[y1],mean2[y1,5],col='blue',lty=3,lwd=1.5)
points(x1[y1],mean4[y1,5],col='black',lty=1)
par(mgp=c(1.5,0.6,0))
axis(side=1, pos = 0.0,cex.axis=1.0,tck=-0.01,at=c(1985,1989,1993,1997,2001,2005,2009,2013), 
     labels=c("1985","1989","1993",'1997','2001','2005','2009','2013'),family = 'serif')
axis(side=2, cex.axis=1.0,tck=-0.01,family = 'serif')
abline(h=0.0)
text(x=1986,y=1.95,labels='Annual Mean',font=2,cex=1.2)
text(x=2013,y=1.95,labels='(d)',font=2,cex=1.2)
#abline(v=2005.5,h=c(0.1,0.9),col='grey30',lty=2)
segments(2005.5, 0.01, x1 = 2005.5, y1 = 2.0,col = 'grey30', lty = 2)
dev.off()